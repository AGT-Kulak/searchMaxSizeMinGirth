...
if __name__ == "__main__":
    ...
    file_dir = f"upRun_iter{iter}_{L}"

    bestGraphsSaveDir = f"bestGraphs/iter{iter}Up_{L}"
    ...
    if iter == 1:
        readStartGraphs("bestGraphs/startGraphs")
    else:
        readStartGraphs(f"bestGraphs/iter{iter-1}Down_{L}")

    lowerOrder = 51
    upperOrder = 203

    orders = range(lowerOrder,upperOrder+1)
    numItersFunc = lambda x : x * 1000
    p = 0.5
    numsDelEdgeFuncs = [lambda x : 3, lambda x : max([x//10,3])]
    input_file_ptg = startGraphFileOfOrder(lowerOrder-1)
    ...
    for n in orders:
        ...
        command = (f"./lib/nauty/addptg -n1 < {input_file_ptg} | ./lib/topLMostEdges {L} > {output_file_ptg}")
        ...
        print(f"Add vertex n={n-1}->{n} done")
